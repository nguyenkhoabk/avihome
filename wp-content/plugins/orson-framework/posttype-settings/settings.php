...
{
    wp_die( esc_html__('You do not have sufficient permissions to access this page.', 'g5plus-orson') );
}
...
if(isset($taxonomy)&& is_array($taxonomy) && count($taxonomy)>0){
	...
	$taxonomy_slug = $taxonomy_objects[$taxonomy_key]->rewrite['slug'];
	$taxonomy_name = $taxonomy_objects[$taxonomy_key]->labels->name;
}
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST[ $post_type.'-slug' ])){
    if($_POST[ $post_type.'-slug' ]!='' || $_POST['category-'. $post_type.'-slug' ]!=''){
        ...
        if(!$is_match_slug || !$is_match_taxonomy_slug){
            update_option('g5plus-orson-'.$post_type.'-config',$config);
            $message = esc_html__('New slug has been update.','g5plus-orson');
            flush_rewrite_rules();
        }else{
            $message = esc_html__('Value not allowed to container special characters ($&* #@%^!~+?) and space.','g5plus-orson');
            $message_class = 'error';
        }
    }else{
        $message = esc_html__('Input value to new slug and name.','g5plus-orson') ;
        $message_class = 'error';
    }
}
...
<form name="frmChangePostTypeSlug" method="post" action="">
    <div class="page-post-type-setting updated">
        <h3>Change Slug</h3>
        <ul>
