...
if ( !function_exists( 'g5plus_get_tax_meta') ) {
	function g5plus_get_tax_meta($term_id,$key,$multi = false) {
		if ( function_exists('get_term_meta')){
			return get_term_meta($term_id, G5PLUS_METABOX_PREFIX . $key, !$multi );
		}else{
			return get_tax_meta( $term_id, G5PLUS_METABOX_PREFIX . $key, !$multi  );
		}
	}
}
...
if (!function_exists('g5plus_maintenance_mode')) {
    function g5plus_maintenance_mode() {
        ...
        switch ($enable_maintenance) {
            case 1 :
                wp_die( '<p style="text-align:center">' . esc_html__( 'We are currently in maintenance mode, please check back shortly.', 'g5plus-orson' ) . '</p>', get_bloginfo( 'name' ) );
                break;
            case 2:
...
if (!function_exists('g5plus_categories_binder')) {
	function g5plus_categories_binder($categories, $parent,$class= 'search-category-dropdown', $is_anchor = false, $show_count = false, $show_all = false) {
		$index = 0;
		$output = '';
		$parent .= '';
			...
			if ($index == 0) {
				...
				if ($show_all) {
					...
					$output .= sprintf('%s%s%s',
						'<span data-id="-1">',
						esc_html__('Select Category','g5plus-orson'),
						'</span>'
					);
					$output .= '</li>';
				}
...
// GET ID BY SLUG
//==============================================================================
if (!function_exists('g5plus_get_id_by_slug')) {
	function g5plus_get_id_by_slug( $slug ) {
		global $wpdb;
...
if (!function_exists('g5plus_output_content_wrapper')) {
	function g5plus_output_content_wrapper(){
		if (is_404()) return;
		g5plus_get_template('global/wrapper-start');
	}
...
if (!function_exists('g5plus_output_content_wrapper_end')) {
	function g5plus_output_content_wrapper_end(){
		if (is_404()) return;
		g5plus_get_template('global/wrapper-end');
	}
