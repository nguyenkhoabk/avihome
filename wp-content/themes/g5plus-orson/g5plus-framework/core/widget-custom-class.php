...
function g5plus_custom_css_in_widget_form( $widget, $return, $instance ) {
	...
	$layout = g5plus_get_widget_layout(true);
	$layout_html = "<p>
			<label for='widget-{$widget->id_base}-{$widget->number}-layout'>". esc_html__( 'Layout', 'g5plus-orson' ) .":</label>
			<select class='widefat' name='widget-{$widget->id_base}[{$widget->number}][layout]' id='widget-{$widget->id_base}-{$widget->number}-layout'>";
	foreach ($layout as $key => $value) {
		$layout_html .= "<option value='{$key}'". selected( $key, $instance['layout'],false ).">{$value}</option>";
	}
	$layout_html .= "
			</select>
		</p>";
	echo $layout_html;
	echo "<p><label for='widget-{$widget->id_base}-{$widget->number}-css_class'>". esc_html__( 'CSS Class', 'g5plus-orson' ) .":</label>
...
function g5plus_custom_css_widget_update_callback( $instance, $new_instance ) {
	...
	$instance['layout'] = array_key_exists('layout',$new_instance) ? $new_instance['layout'] : '';
	return $instance;
}
...
function g5plus_custom_css_add_widget_css_class( $params ) {
	...
	if ( isset( $widget_opt[$widget_num]['layout'] ) && !empty( $widget_opt[$widget_num]['layout'] ) ) {
		...
		$params[0]['before_widget'] = preg_replace( '/class="'.$widget_layout.'/', "class=\"{$widget_opt[$widget_num]['layout']} ", $params[0]['before_widget'], 1 );
	}
