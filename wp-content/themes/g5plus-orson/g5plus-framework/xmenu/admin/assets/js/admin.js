...
(function($) {
	...
	var XMENU = {
		...
		reset: function() {
			$('.xmenu-config-panel-wrapper li.x-reset').on('click',function(){
		...
		save_config_panel: function() {
			$('.xmenu-header .xmenu-config-panel-save').on('click',function() {
				if ($('.xmenu-header .xmenu-config-panel-save > i').hasClass('fa-spinner')) {
					return;
				}
				$('> i', this).attr('class','fa fa-spinner fa-spin');
		...
		xmenu_config_panel_close_click: function() {
			$('.xmenu-header > h2 .xmenu-config-panel-close').on('click',function(){
		...
		rel_section_click:function() {
			$('.xmenu-config-panel-left > ul > li[rel-section]').on('click',function(){
		...
		xmenu_icon_popup_event: function(){
			$('.x-icon-wrapper').on('click',function(event){
			...
			$('.xmenu-icon-popup-header > i.fa-remove').on('click',function(){
				$('.xmenu-icon-popup').fadeOut();
			});
			$('.xmenu-icon-popup-content > ul > li').on('click',function(){
			...
			$('.x-icon-remove').on('click',function(){
			...
			$('.xmenu-icon-popup .xmenu-icon-remove').on('click',function(){
				var data_rel = $('.xmenu-icon-popup').attr('data-rel');
			...
			$('.xmenu-icon-popup-header > input').on('keyup',function(){
				var filter = $(this).val();
