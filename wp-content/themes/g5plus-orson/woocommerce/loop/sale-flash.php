...
global $post, $product;
// product sale label
$product_sale_label_enable = G5Plus_Global::get_option('product_sale_label_enable');
if ($product->is_on_sale() && $product->product_type != 'grouped' && $product_sale_label_enable && $product->get_regular_price() > 0) {
	$product_sale_flash_mode = G5Plus_Global::get_option('product_sale_flash_mode');
	if ($product_sale_flash_mode == 'text') {
		$product_sale_label_text = G5Plus_Global::get_option('product_sale_label_text');
	} else {
		if ($product->product_type == 'variable') {
			...
			for ($i = 0; $i < count($available_variations); ++$i) {
				...
				if ( $sales_price != $regular_price && $sales_price == $price && $regular_price > 0  ) {
...
$product_featured_label_enable = G5Plus_Global::get_option('product_featured_label_enable');
if ($product->is_featured() && $product_featured_label_enable) {
	$product_featured_label_text = G5Plus_Global::get_option('product_featured_label_text');
